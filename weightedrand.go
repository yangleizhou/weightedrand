...
// Pick several without duplication.
func (chs Chooser) PickN(n int) []interface{} {
	...
	for {
		r := rand.Intn(chs.max) + 1
		i := sort.SearchInts(chs.totals, r)
		if !chs.data[i].Selected {
			ret = append(ret, chs.data[i].Item)
			chs.data[i].Selected = true
			length += 1
			usingTotal += int(chs.data[i].Weight)
			if length >= n || float64(usingTotal)/float64(chs.max) > 0.75 {
				break
			}
		}
	}
